...
#define PBJVisionThumbnailWidth [[UIScreen mainScreen] bounds].size.width

static BOOL kExistsRecordEnine = NO;
// KVO contexts
...
AVCaptureVideoDataOutputSampleBufferDelegate>
{
    ...
    AVCaptureVideoPreviewLayer *_bjPreviewLayer;
...
@synthesize bjPreviewLayer = _bjPreviewLayer;
...
- (Float64)capturedVideoSeconds
{
    /*
     ...
     NSLog(@"_startTimestamp%f",CMTimeGetSeconds(_startTimestamp));
     NSLog(@"currentTime%f",CMTimeGetSeconds(currentTime));
     Float64 timeDiff = CMTimeGetSeconds(CMTimeSubtract(currentTime, _startTimestamp));
     NSLog(@"Time difference = %f",timeDiff);
     
     return timeDiff;
     */
    
    if (_videoTimestamp.value > 0) {
        Float64 ss = CMTimeGetSeconds(CMTimeSubtract(_videoTimestamp, _startTimestamp));
        return ss;
    } else {
        return 0.0;
    }
...
- (void)_setOrientationForConnection:(AVCaptureConnection *)connection
{
    ...
    else if (_cameraDevice == PBJCameraDeviceBack) {
        if (connection.isVideoMirrored) {
...
- (void)_setCameraMode:(PBJCameraMode)cameraMode cameraDevice:(PBJCameraDevice)cameraDevice outputFormat:(PBJOutputFormat)outputFormat
{
    ...
    if (cameraMode == PBJCameraModePhoto) {
        _flashMode = PBJFlashModeAuto;
    }
    else {
        _flashMode = PBJFlashModeOff;
    }
...
- (id)initWithShowGLContext:(BOOL)isShowGLContext
{
    self = [super init];
    if (self) {
        kExistsRecordEnine = YES;
        if (isShowGLContext) {
            _context = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2];
            if (!_context) {
                DLog(@"failed to create GL context");
            }
            [self _setupGL]; //delete by zhangyx
        }
        
//        _audioAssetBitRate = 64000;
        _audioAssetBitRate = kAudioAssetBitRate;
        ...
        _videoAssetFrameInterval = kTimeScale;
        _timeScale = kTimeScale;
        ...
        _captureSessionPreset = kAVCaptureSessionPreset;
        ...
        _bjPreviewLayer = [[AVCaptureVideoPreviewLayer alloc] initWithSession:nil];
...
- (void)dealloc
{
    ...
    kExistsRecordEnine = NO;
    _delegate = nil;

    if (_cameraMode == PBJCameraModeVideo && self.isCanReleaseContext) {
...
- (BOOL)_setupCamera
{
    ...
    if (error) {
        DLog(@"_setupCamera error setting up front camera input (%@)", error);
        error = nil;
        // 代理回调 无法使用正面相机
        if ([self.delegate respondsToSelector:@selector(cannotGetFrontCamera:)]) {
            [self.delegate cannotGetFrontCamera:error];
        }
    }
    
    _captureDeviceInputBack = [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceBack error:&error];
    if (error) {
        DLog(@"_setupCamera error setting up back camera input (%@)", error);
        error = nil;
        // 代理回调，无法使用背面相机
        if ([self.delegate respondsToSelector:@selector(cannotGetBackCamera:)]) {
            [self.delegate cannotGetBackCamera:error];
        }
        return NO;
    }
    ...
    if (self.cameraMode != PBJCameraModePhoto) {
        ...
        if (error) {
            DLog(@"_setupCamera error setting up audio input (%@)", error);
            // 无法使用“iPhone 麦克风
            if ([self.delegate respondsToSelector:@selector(cannotGetAudioDevice:)]) {
                [self.delegate cannotGetAudioDevice:error];
            }
            return NO;
        }
    }
...
- (void)_destroyCamera
{
    ...
    */
    ...
    if (_captureOutputAudio) {
        [_captureOutputAudio setSampleBufferDelegate:nil queue:nil];
    }
    if (_captureOutputVideo) {
        [_captureOutputVideo setSampleBufferDelegate:nil queue:nil];
    }
...
- (void)_setupSession
{
    ...
    [_captureSession setSessionPreset:kAVCaptureSessionPreset];
    ...
    */
    ...
    if (newCaptureOutput && newCaptureOutput == _captureOutputVideo && videoConnection) {
        ...
        sessionPreset = kAVCaptureSessionPreset;
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
            ...
            if ([newCaptureDevice lockForConfiguration:&error]) {
                ...
                // focus mode add by zhangyx
                if ([newCaptureDevice isFocusModeSupported:_focusMode]) {
                    [newCaptureDevice setFocusMode:_focusMode];
                }
                
                // setup framerate range
                // TODO: seek best framerate range for slow-motion recording
                CMTime frameDuration = CMTimeMake( 1, _timeScale );
...
#pragma clang diagnostic pop
        ...
        // setup photo settings modify by zhangyx
        NSDictionary *photoSettings = [[NSDictionary alloc] initWithObjectsAndKeys:
                                       AVVideoCodecJPEG, AVVideoCodecKey,
                                       [NSNumber numberWithFloat:PBJVisionThumbnailWidth],AVVideoHeightKey,
                                       [NSNumber numberWithFloat:PBJVisionThumbnailWidth],AVVideoWidthKey,
                                       nil];
...
- (void)startPreview
{
    [self _enqueueBlockInCaptureSessionQueue:^{
        ...
        // add by zhangyx
        if (_bjPreviewLayer && _bjPreviewLayer.session != _captureSession) {
            _bjPreviewLayer.session = _captureSession;
        }
...
- (void)stopPreview
{
    [self _enqueueBlockInCaptureSessionQueue:^{
        ...
        // add by zhangyx
        if (_bjPreviewLayer) {
            _bjPreviewLayer.connection.enabled = YES;
        }
        
        if (_captureSession.isRunning) { // modify by zhangyx
...
- (void)freezePreview
{
    ...
    if (_bjPreviewLayer) {
        _bjPreviewLayer.connection.enabled = NO;
    }
}
...
{
    AVCaptureDevice *device = _currentDevice;
    ...
    if ([device isFocusPointOfInterestSupported] /*&& [device isFocusModeSupported:AVCaptureFocusModeAutoFocus]*/) {
        ...
        if ([device lockForConfiguration:&error]) {
            ...
            NSLog(@"didFailWithError:setFocusPointOfInterestWithAVCaptureFocusModeAutoFocus");
        }
    }
}
...
- (void)continuousFocusAtPoint:(CGPoint)point
{
    ...
    if ([device isFocusPointOfInterestSupported] && [device isFocusModeSupported:AVCaptureFocusModeContinuousAutoFocus]) {
		...
		if ([device lockForConfiguration:&error]) {
            ...
            NSLog(@"didFailWithError:setFocusPointOfInterestWithAVCaptureFocusModeContinuousAutoFocus");
		}
	}
}
/********add by zhangyx *********/
...
- (void)focusAtAdjustedPoint:(CGPoint)adjustedPoint
{
    ...
    if ([_currentDevice lockForConfiguration:&error]) {
        ...
        if (isFocusAtPointSupported && [_currentDevice isFocusModeSupported:_focusMode]) {
            [_currentDevice setFocusPointOfInterest:adjustedPoint];
            [_currentDevice setFocusMode:_focusMode];
        }
...
- (NSArray *)_metadataArray
{
    ...
    [softwareItem setValue:[NSString stringWithFormat:@"%@ %@", [currentDevice systemName], [currentDevice systemVersion]]];
...
- (void)startVideoCapture
{
    ...
    NSError *error;
    [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceFront error:&error];
    if (error) {
        DLog(@"startVideoCapture error setting up front camera input (%@)", error);
        error = nil;
        // 代理回调 无法使用正面相机
        if ([self.delegate respondsToSelector:@selector(cannotGetFrontCamera:)]) {
            [self.delegate cannotGetFrontCamera:error];
    ...
    NSError *videoError;
    [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceBack error:&videoError];
    if (videoError) {
        DLog(@"startVideoCapture error setting up back camera input (%@)", videoError);
        // 代理回调，无法使用背面相机
        if ([self.delegate respondsToSelector:@selector(cannotGetBackCamera:)]) {
            [self.delegate cannotGetBackCamera:videoError];
        }
        return ;
    }

    NSError *audioError;
    [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceAudio error:&audioError];
    if (audioError) {
        DLog(@"startVideoCapture error setting up audio input (%@)", audioError);
        // 无法使用“iPhone 麦克风
        if ([self.delegate respondsToSelector:@selector(cannotGetAudioDevice:)]) {
            [self.delegate cannotGetAudioDevice:audioError];
        }
        return;
    }
    ...
    [self _enqueueBlockInCaptureVideoQueue:^{
        ...
        NSString *outputPath = [self.outputURL path];//[NSString stringWithFormat:@"%@%@", NSTemporaryDirectory(), @"video.mp4"];
        ...
        NSError *error = nil;
        _assetWriter = [[AVAssetWriter alloc] initWithURL:self.outputURL fileType:(NSString *)kUTTypeMPEG4 error:&error];
        if (error) {
    ...
    [self _enqueueBlockInCaptureVideoQueue:^{
        ...
        void (^finishWritingCompletionHandler)(void) = ^{
            ...
            [self _enqueueBlockOnMainQueue:^{
                ...
                [videoDict setObject:[self.outputURL path] forKey:PBJVisionVideoPathKey];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceFront error:&error];
    if (error) {
        DLog(@"captureOutput error setting up front camera input (%@)", error);
        error = nil;
        // 代理回调 无法使用正面相机
//        if ([self.delegate respondsToSelector:@selector(cannotGetFrontCamera:)] && !_flags.showCannotGetFrontCameraError) {
//            _flags.showCannotGetFrontCameraError = YES;
//            [self.delegate cannotGetFrontCamera:error];
//        }
    }
    ...
    [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceBack error:&videoError];
    if (videoError) {
        DLog(@"captureOutput error setting up back camera input (%@)", videoError);
        // 代理回调，无法使用背面相机
//        if ([self.delegate respondsToSelector:@selector(cannotGetBackCamera:)] && !_flags.showCannotGetBackCameraError) {
//            _flags.showCannotGetBackCameraError = YES;
//            [self.delegate cannotGetBackCamera:videoError];
//        }
        return ;
    }
    ...
    [AVCaptureDeviceInput deviceInputWithDevice:_captureDeviceAudio error:&audioError];
    if (audioError) {
        DLog(@"captureOutput error setting up audio input (%@)", audioError);
        // 无法使用“iPhone 麦克风
//        if ([self.delegate respondsToSelector:@selector(cannotGetAudioDevice:)] && !_flags.showCannotGetAudioDeviceError) {
//            _flags.showCannotGetAudioDeviceError = YES;
//            [self.delegate cannotGetAudioDevice:audioError];
//        }
        return;
    }// add by zhangyx
...
{
    DLog(@"session interruption will ended");
    if (!kExistsRecordEnine) {
        return;
    }
    [self _enqueueBlockOnMainQueue:^{
...
{
    DLog(@"_inputPortFormatDescriptionDidChange");
...
{
    DLog(@"_deviceSubjectAreaDidChange");
    if (!kExistsRecordEnine) {
        return;
    }
    [self _focus];
