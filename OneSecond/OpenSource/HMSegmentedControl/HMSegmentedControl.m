...
//            CGFloat stringWidth = roundf([titleString sizeWithFont:self.font].width);
            ...
            if (self.segmentWidthStyle == HMSegmentedControlSegmentWidthStyleFixed) {
                tRect = CGRectMake((self.segmentWidth * idx) + (self.segmentWidth - stringWidth)/2, y, stringWidth, stringHeight);
            }
                ...
                tRect = CGRectMake(xOffset, y, [[self.segmentWidthsArray objectAtIndex:idx] floatValue], stringHeight);
            }
...
//=========================Add by Amy===============================================================
            ...
            rightBorderLayer.backgroundColor = [UIColor colorWithRed:138/255.0 green:157/255.0 blue:172/255.0 alpha:1].CGColor;
            rightBorderLayer.frame = CGRectMake(self.segmentWidth * (idx + 1), 0, 0.5, self.scrollView.frame.size.height);
            [self.scrollView.layer addSublayer:rightBorderLayer];
//==============================End=================================================================
        }];
...
//============================== Add by Amy ========================================================
        else if (self.selectionStyle == HMSegmentedControlSelectionStyleCustomWidthStripe && sectionWidth <= self.segmentWidth) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
...
//=========================Add by Amy===============================================================
            if (self.customSegmentWidth > 0) {
                self.segmentWidth = self.customSegmentWidth;
            }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(self.bounds, touchLocation)) {
        ...
        if (segment >= [self sectionCount]) {
            return;
        }
        if (segment != self.selectedSegmentIndex) {
            // Check if we have to do anything with the touch event
            if (self.isTouchEnabled)
                [self setSelectedSegmentIndex:segment animated:_touchIsAnimate notify:YES];
        }
    }
}
