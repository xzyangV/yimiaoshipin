...
-(void)tapClick{
    NSLog(@"%@",[_pages[self.currentPageIndex] urlStr]);
    
    if ([(id)self.delegate respondsToSelector:@selector(intoPlayVideoWithUrlStr:)]) {
        [self.delegate intoPlayVideoWithUrlStr:[_pages[self.currentPageIndex] urlStr]];
    }
}
