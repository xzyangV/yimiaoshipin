...
//            [self.mainWindow makeKeyAndVisible];
    ...
    if (self.completion) {
        BOOL cancelled = NO;
        if (sender == self.cancelButton || (sender == self.tap && !self.tapBlankNotCancel)) {
            cancelled = YES;
        }
...
//    lineLayer.backgroundColor = [[UIColor colorWithWhite:0.90 alpha:0.3] CGColor];
    lineLayer.backgroundColor = [UIColor colorWithWhite:0.3 alpha:0.3].CGColor;
    return lineLayer;
}
...
{
    return [PXAlertView showAlertWithTitle:title message:nil cancelTitle:@"确认" completion:nil];
}

+ (instancetype)showAlertWithTitle:(NSString *)title
                           message:(NSString *)message
{
    return [PXAlertView showAlertWithTitle:title message:message cancelTitle:@"确认" completion:nil];
}
...
+ (instancetype)showAlertWithTitle:(NSString *)title
                        ...
                        completion:(PXAlertViewCompletionBlock)completion
{
    return [PXAlertView showAlertWithTitle:title message:message cancelTitle:@"确认" completion:completion];
}
...
+ (instancetype)showAlertWithCustomView:(UIView *)customView
                             completion:(PXAlertViewCompletionBlock)completion
{
    PXAlertView *alertView = [[PXAlertView alloc] initWithTitle:nil message:nil cancelTitle:nil otherTitles:nil contentView:customView isCustom:YES completion:completion];
    [alertView show];
    return alertView;
}
...
- (NSInteger)addButtonWithTitle:(NSString *)title
{
    ...
    UIFont *btnTitleFont = [UIFont systemFontOfSize:17];
    ...
    if (!self.cancelButton) {
        ...
        lastButton.titleLabel.font = btnTitleFont;
        ...
        self.cancelButton.titleLabel.font = btnTitleFont;
    }
